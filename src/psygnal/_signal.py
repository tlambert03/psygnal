...
from psygnal._weak_caller import WeakCallback, _SetattrCaller, _SetitemCaller
...
class SignalInstance:
    ...
    def __repr__(self) -> str:
        ...
        instance = f" on {_instance!r}" if _instance is not None else ""
        ...
        * if `check_types` is `True`, the parameter types in the callback signature must
        ...
        def _wrapper(slot: Callable, max_args: int | None = max_args) -> Callable:
            ...
            with self._lock:
                ...
                self._slots.append(WeakCallback.create(slot, max_args=max_args))
            return slot
        ...
        with self._lock:
            ...
            for i, slot in enumerate(self._slots):
                if isinstance(slot, _SetitemCaller):
                    if slot._obj_ref() is obj and slot._key == key:
                        idx = i
                        break
            if idx is not None:
                self._slots.pop(idx)
    ...
    def _run_emit_loop(self, args: tuple[Any, ...]) -> None:
        rem: list[WeakCallback] = []
        # allow receiver to query sender with Signal.current_emitter()
        with self._lock:
            with Signal._emitting(self):
                for caller in self._slots:
                    try:
                        if caller.callback(args):
                            # if the WeakCallback returns `True`, the object weakref is
